...
if "%GITHUB_WORKFLOW%" neq "" (
  ...
  python ..\mesa.src\src\vulkan\util\vk_icd_gen.py --api-version 1.1 --xml ..\mesa.src\src\vulkan\registry\vk.xml --lib-path vulkan_lvp.dll --out lvp_icd.x86_32.json
  copy /y ..\mesa-llvmpipe\share\vulkan\icd.d\lvp_icd.x86_32.json .
  ...
  if exist "%ProgramFiles(x86)%\Windows Kits\10\Redist\D3D\x32\dxil.dll" (
    copy /y "%ProgramFiles(x86)%\Windows Kits\10\Redist\D3D\x32\dxil.dll" .
  ) else if exist "%WindowsSdkVerBinPath%x32\dxil.dll" (
    copy /y "%WindowsSdkVerBinPath%x32\dxil.dll" .
  )
